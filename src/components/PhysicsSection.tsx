...
export const PhysicsSection: React.FC<PhysicsSectionProps> = ({ searchQuery }) => {
  const [selectedChip, setSelectedChip] = useState<string>('all');
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const filteredFormulas = useMemo(() => {
    return PHYSICS_FORMULAS.filter((item) => {
      // Task filter match
      ...
      } else if (selectedChip === 'mech') {
        matchesFilter = item.taskNumber >= 1 && item.taskNumber <= 6;
      } else if (selectedChip === 'mkt') {
        matchesFilter = item.taskNumber >= 7 && item.taskNumber <= 10;
      } else if (selectedChip === 'electro') {
        matchesFilter = item.taskNumber >= 11 && item.taskNumber <= 15;
      } else if (selectedChip === 'quantum') {
        matchesFilter = item.taskNumber >= 16 && item.taskNumber <= 17;
      ...
      const matchesSearch =
        ...
        item.variables.toLowerCase().includes(query) ||
  ...
  const copyFormula = (item: PhysicsFormula) => {
    ...
    <div className="space-y-6">
      ...
      {filteredFormulas.length === 0 ? (
        <div className="text-center py-16 bg-[#252538] rounded-3xl p-8 border border-[#313244]/40">
          ...
          <p className="text-[#a6adc8] text-sm mt-1">Попробуйте изменить категорию или поисковый запрос</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filteredFormulas.map((item) => {
            const isCopied = copiedId === item.id;
            const isJustifyItem = item.category.includes('Обоснование') || item.taskNumber === 26;
            ...
            return (
              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {item.isCrucial && (
                        <span className="px-3 py-1 rounded-full text-[11px] font-semibold tracking-wider uppercase bg-[#fab387]/20 text-[#fab387]">
                          Ключевое правило
                        </span>
                      )}
                ...
                {/* Description, Variables & Reference link */}
                <div className="space-y-3 text-xs text-[#bac2de] mt-2">
                  ...
                  <div className="text-[#a6adc8] bg-[#1e1e2e]/50 p-3 rounded-xl leading-relaxed flex items-start gap-2">
                    <Info className="w-4 h-4 text-[#89dceb] shrink-0 mt-0.5" />
